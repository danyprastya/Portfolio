...
import { UploadCloud, CheckCircle, XCircle, File, X } from "lucide-react"
...
export default function FileUpload({
  ...
  currentFiles = [], // Changed from currentFile to currentFiles array
  onFileRemove = () => {},
  uploadDelay = 2000,
  validateFile = () => null,
  className,
  ...
  const simulateUpload = useCallback(
    (uploadingFile: File) => {
      ...
      if (uploadDelay === 0) {
        setStatus("success")
        setSuccessMessage(`${uploadingFile.name} attached successfully!`)
      ...
      uploadIntervalRef.current = setInterval(
        () => {
          ...
          if (currentProgress >= 100) {
            ...
            setProgress(0)
            setStatus("success")
            setSuccessMessage(`${uploadingFile.name} uploaded successfully!`)
            onUploadSuccess?.(uploadingFile)
  ...
  const AttachedFilesList = ({ files }: { files: File[] }) => {
        ...
        <div className="space-y-1 max-h-20 overflow-y-auto">
          {files.map((file, index) => (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="flex items-center gap-2 p-1.5 bg-gray-50 dark:bg-gray-800/50 rounded-md border border-gray-200 dark:border-gray-700"
            >
  ...
  return (
    ...
    >
      <AttachedFilesList files={currentFiles} />
              ...
              <AnimatePresence mode="wait">
                {status === "idle" || status === "dragging" ? (
                  ...
                  >
                    ...
                    <div className="text-center space-y-1 mb-2">
                      <h3 className="text-sm font-semibold text-gray-900 dark:text-white tracking-tight">
                        {currentFiles.length > 0 ? "Add another file" : "Drag and drop or"}
                      </h3>
                    ...
                    <button
                      ...
                      className="w-4/5 flex items-center justify-center gap-2 rounded-lg bg-gray-100 dark:bg-white/10 px-3 py-2 text-xs font-semibold text-gray-900 dark:text-white transition-all duration-200 hover:bg-gray-200 dark:hover:bg-white/20 group"
                    >
                      <span>{currentFiles.length > 0 ? "Add File" : "Upload File"}</span>
                      <UploadCloud className="w-3 h-3 group-hover:scale-110 transition-transform duration-200" />
                    </button>

                    <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">or drag and drop your file here</p>
                  ...
                  >
                    ...
                    <div className="text-center space-y-1 mb-2">
                      <h3 className="text-xs font-semibold text-gray-900 dark:text-white truncate">{file?.name}</h3>
                    ...
                    <button
                      ...
                      type="button"
                      className="w-4/5 flex items-center justify-center gap-2 rounded-lg bg-gray-100 dark:bg-white/10 px-3 py-2 text-xs font-semibold text-gray-900 dark:text-white transition-all duration-200 hover:bg-gray-200 dark:hover:bg-white/20"
                    >
                      Cancel
                    </button>
                  </motion.div>
                  ...
                  >
                    ...
                    <div className="text-center space-y-1 mb-2">
                      <h3 className="text-sm font-semibold text-green-600 dark:text-green-400">File Attached!</h3>
                      <p className="text-xs text-gray-600 dark:text-gray-300 truncate max-w-full">{successMessage}</p>
                    </div>

                    <button
                      onClick={resetUploadState}
                      type="button"
                      className="w-4/5 flex items-center justify-center gap-2 rounded-lg bg-green-100 dark:bg-green-900/20 px-3 py-2 text-xs font-semibold text-green-700 dark:text-green-300 transition-all duration-200 hover:bg-green-200 dark:hover:bg-green-900/30"
                    >
                      Add Another File
                    </button>
                  </motion.div>
            ...
            <AnimatePresence>
              {error && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  className="absolute bottom-4 left-1/2 transform -translate-x-1/2 px-4 py-2 bg-red-500/10 border border-red-500/20 rounded-lg flex items-center gap-2"
                >
