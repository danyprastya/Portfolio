...
export default function AnimatedInput({
  ...
  return (
    <div className={`relative flex items-center ${className}`}>
      ...
      />
      <motion.label
        className={`bg-background text-foreground pointer-events-none absolute  left-2 origin-left  rounded-sm border border-transparent px-1 transition-all ${labelClassName}`}
        animate={
          isFloating
            ? {
                y: -24,
